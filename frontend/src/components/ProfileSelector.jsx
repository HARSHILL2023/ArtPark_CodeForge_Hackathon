  ...
  return (
    <div className="space-y-1.5">
      <label className="flex items-center gap-1.5 text-[11px] font-semibold text-[#5E5C56] dark:text-[#B4B1A9]">
        <Briefcase className="w-3.5 h-3.5 text-[#2563EB] dark:text-[#3B82F6]" />
        <span>Pre-loaded Benchmark Profiles</span>
      </label>
      <select
        value={selectedProfile}
        onChange={(e) => onProfileChange?.(e.target.value)}
        className="w-full px-3 py-2 bg-[#FCFBF8] dark:bg-[#181B1F] border border-[#DCD9D1] dark:border-[#292D33] rounded-xl text-xs text-[#1B1B19] dark:text-[#F2F0EA] focus:outline-none focus:ring-1 focus:ring-[#2563EB] dark:focus:ring-[#3B82F6] transition-colors cursor-pointer"
      >
        {options.map((option) => (
